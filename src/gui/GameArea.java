...
public class GameArea extends JPanel implements MummyMazeListener {
		...
		for(int i = 0; i < 13; i++) {
			for(int j = 0; j < 13; j++) {				
				switch(matrix[i][j]) {
					...
					//Show the exit image on background and hero image on foreground, for when the hero is on top of the exit
					//F, as in "Final"
					case 'F' :
	...
	public MummyMazeState getState () {
		return this.state;
	}
	...
	@Override
	public void heroChanged(MummyMazeEvent pe) {
		solutionCost++;
	...
	@Override
	public void enemyChanged(MummyMazeEvent pe) {
		repaint();
		try {
			Thread.sleep(500);
		} catch (InterruptedException ignore) {
